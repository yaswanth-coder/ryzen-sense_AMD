"""
RyzenSense v2 - Profile Agent (Groq Version)
"""
...
BUILTIN_PRESETS = {
    ...
    "silent": {
        "profile_name": "silent",
        "reasoning": "Minimize noise and heat.",
...
class ProfileAgent(BaseAgent):
    ...
    def recommend(self, context: dict) -> str:
        ...
        prompt = f"""Based on this hardware context, recommend ONE profile name from:
...
Return ONLY the profile name."""
    ...
    def apply_profile(self, name: str) -> bool:
        ...
        self.broadcast("tune_applied", {"goal": f"profile:{name}", "settings": settings, "profile": name})
        return True
        ...
        try:
            with sqlite3.connect(DB_PATH) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    data.get("timestamp"), data.get("single_thread_score", 0),
                    data.get("multi_thread_score", 0), data.get("mt_efficiency", 0),
                    data.get("cpu_temp_before", 0), data.get("cpu_temp_after", 0),
                ))
        except Exception as e:
    ...
    def show_history(self, limit: int = 20):
        try:
            with sqlite3.connect(DB_PATH) as conn:
                rows = conn.execute("""
                    SELECT timestamp, goal, profile_name, tdp_applied_w, cpu_temp_before
